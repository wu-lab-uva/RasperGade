...
all.data = readRDS("landis.data.unlist.RDS")[readRDS("landis.PE.index.RDS")]
raw.model = readRDS("landis.PE.combined.normal.models.RDS")
...
#
if(file.exists("Landis.ASR.PEonly.CV.RDS")){
  all.asr = readRDS("Landis.ASR.PEonly.CV.RDS")
}else{
  all.asr = list()
  ...
  saveRDS(all.asr,"Landis.ASR.PEonly.CV.RDS")
}
...
saveRDS(PEBME.pZ,"Landis.PEBME.plot.data.RDS")
#
